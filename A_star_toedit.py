...
class AStar(object):
    ...
    def is_free(self, x):
        ...
        if (self.occupancy.is_free(x)):
            lo = self.statespace_lo
            hi = self.statespace_hi
            if x[0] > lo[0] and x[1] > lo[1] and x[1] < hi[1] and x[0] < hi[0]:
                return True
        else:
            return False
    ...
    def get_neighbors(self, x):
        ...
        sec =x[1]
        ########## Code starts here ##########
        neighbors.append(self.snap_to_grid((fir + self.resolution,sec)))
        neighbors.append(self.snap_to_grid((fir - self.resolution, sec)))
        neighbors.append(self.snap_to_grid((fir, sec + self.resolution)))
        neighbors.append(self.snap_to_grid((fir, sec - self.resolution)))
        neighbors.append(self.snap_to_grid((fir - self.resolution, sec - self.resolution)))
        neighbors.append(self.snap_to_grid((fir + self.resolution, sec + self.resolution)))
        neighbors.append(self.snap_to_grid((fir - self.resolution, sec + self.resolution)))
        neighbors.append(self.snap_to_grid((fir + self.resolution, sec - self.resolution)))
        lst_tmp =[]
        for i in range(len(neighbors)):
            if self.is_free(neighbors[i]):
                lst_tmp.append(neighbors[i])
    ...
    def solve(self):
        ...
        while len(self.open_set) > 0:
            ...
            for neigh in self.get_neighbors(x_curr):
                ...
                tent = self.cost_to_arrive[x_curr] + self.distance(x_curr,neigh)
                if neigh not in self.open_set:
                    self.open_set.add(neigh)
                
                self.came_from[neigh] = x_curr
                self.cost_to_arrive[neigh] = tent
                self.est_cost_through[neigh] = tent + self.distance(neigh,self.x_goal)
