...
class RRT(object):
    ...
    def is_free_motion(self, obstacles, x1, x2):
        ...
        Output:
            Boolean True/False
        """
        return GeometricRRT.is_free_motion(obstacles,x1,x2)
    ...
    def steer_towards(self, x1, x2, eps):
        ...
        """
        dis = np.subtract(x2,x1)
        if dis[0] == 0:
            dis[0] == 0.5
            dir = round(np.arctan(dis[1]/0.5), 1)
        else:
            dir = round(np.arctan(dis[1]/dis[0]), 1)
        e_dis = round(((dis[0])**(2) +(dis[1])**(2))**(1/2), 1)
        if e_dis < eps:
            return x2
        else:
            y_v = round((np.sin(dir) * eps),1)
            x_v = round((np.cos(dir) * eps),1)
            fin = np.array([x_v,y_v])
    ...
    def solve(self, eps, max_iters=1000, goal_bias=0.05, shortcut=False):
        ...
        for i in range(max_iters):
            u_rand = np.random.uniform(size=1)
            if u_rand < goal_bias:
                x_rand = self.x_goal
            else:
                x_b = self.statespace_lo
                y_b = self.statespace_hi
                round(np.random.uniform(x_b[0],y_b[0]), 2)
                rand1 =  round(np.random.uniform(x_b[0],y_b[0]), 2)
                rand2 =  round(np.random.uniform(x_b[1],y_b[1]), 2)
                x_rand = np.array([rand1,rand2])
            x_near = self.find_nearest(V[:n],x_rand)
            x_new = self.steer_towards(V[x_near],x_rand,eps)
            if self.is_free_motion(self.obstacles,V[x_near],x_new):
                V[n,:] = x_new
    ...
    def shortcut_path(self):
        ...
        while not success:
            success = True
            count=1
            while not np.array_equal(self.path[count],self.x_goal):
                if self.is_free_motion(self.obstacles, self.path[count - 1], self.path[count + 1]):
                    del self.path[count]
                    success = False
                else:
                    count +=1 
...
class GeometricRRT(RRT):
    ...
    def find_nearest(self, V, x):
        ...
        return RRT.find_nearest(self,V,x)
        ########## Code ends here ##########

    def steer_towards(self, x1, x2, eps):
        ########## Code starts here ##########
        # Hint: This should take one line.
        return RRT.steer_towards(self,x1,x2,eps)
        ########## Code ends here ##########
